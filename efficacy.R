...
comb$TRT01P <- unlist(comb$X4)
comb$X2 <- unlist(comb$X2)
comb$X3 <- unlist(comb$X3)
...
tab_html <- df %>% 
  ...
  tab_source_note(
    source_note = paste('Program Source: efficacy.R            Executed: (Draft)',  the_date)
  ) %>%
...
tab_html %>%
  gtsave("efficacy.html", path = "C:\\efficacy" )
